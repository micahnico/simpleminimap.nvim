...
local function render_minimap(buf_nr)
  ...
  fn.execute("normal! Go"..tostring(recent_cache[buf_nr])) -- add to minimap
...
local function on_move()
  local minimap_win_nr = fn.bufwinnr('-MINIMAP-')
  if handling_move or rendering or minimap_win_nr == -1 or is_blocked(bufopt.filetype, bufopt.buftype) then
    return
  end
